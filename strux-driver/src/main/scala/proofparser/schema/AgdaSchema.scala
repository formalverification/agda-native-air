...
/** ------------------------------------------
  ...
  *  ------------------------------------------ */
sealed trait DeclKind { def asString: String }
...
object DeclKind {
  ...
  val all: List[DeclKind] = List(
    Definition, Postulate, Data, Record, Module,
  ...
  implicit val rw: ReadWriter[DeclKind] =
    upickle.default.readwriter[ujson.Value].bimap[DeclKind](
      dk => dk.asString,
...
object SemanticInfo { implicit val rw: ReadWriter[SemanticInfo] = macroRW }
