...
+  `RunningInfo` lines carry Agda's `Checking M (path).` progress, indented by import depth — the same lines `AgdaMCP.Agda.progressModules` already parses from batch output, and the lane's evidence for "this call re-checked the file from source".
...
+  `AgdaMCP.Tools.ProofState` — unchanged in this wave's mandatory scope; its `get_goal` injection path is the designated Phase-3 beneficiary of `Cmd_goal_type_context` (§ 2.5), with the injection kept as fallback.
...
+  `type_of(filePath, expr, line?)` → `Cmd_infer` at the goal whose range contains `line` when one does, else `Cmd_infer_toplevel`; `Normalised` rewrite.  Answers for expressions not present in the file.
+  `normalize(filePath, expr, line?)` → `Cmd_compute` / `Cmd_compute_toplevel`, `DefaultCompute`.
+  `resolve_name(filePath, name, line?)` → `Cmd_why_in_scope` (goal-scoped when `line` addresses a goal, else toplevel), parsed into candidates `{name, qualified?, kind?, provenance chain with file/line/col steps, definition site}`; on a not-in-scope answer the ambiguity recovery of § 2.6 runs before the tool reports an empty candidate list.
+  `definition_of(filePath, name)` → the `resolve_name` machinery, projected to each candidate's `its definition at` step: qualified name, file, line, column.
