...
1.  Agda itself is implemented in Haskell, so the upcoming version of the MCP server
    will be able to call AgdaDojang's Haskell API directly with no subprocess boundary (the
