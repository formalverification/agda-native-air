...
+  **Explicit batch verdict and contract** (#72, open — next batch).  Verification refuted the "check_file green on unsolved metas" claim — the server was already batch-strict — but nothing said so, and an agent chooses tools by reading two-line descriptions.  The work is to echo the verdict, exit code, and resolved command line in every response, and rewrite the tool descriptions to carry the client-visible contract.
