...
+  **Whole-project check tool** (#78, PR 98, open).  The field session's real gate was a 10–20 minute `make check` run four times as backgrounded Bash, grepping logs because a wrapper masked the exit code.  `check_project` runs the project's own gate — the nearest Makefile's `check` target, a command the operator names with `--check-command`, or `agda` on the project's `Everything` module — and reports its verdict without ever misreporting its exit code.  `success` is a conjunction (exit 0, inside the bound, no error diagnostic in the output) precisely so the wrapper trap cannot pass: exit 0 with errors in the log comes back as `maskedFailure`, which is the grep, done once and in the right place.  The response carries the structured first error, the module the gate stopped in, how many modules were rebuilt, and a bounded output tail for the failures Agda never printed.  The gate is bounded by its own `--check-timeout` (default 1800 s, separate from the per-call `--timeout`) and killed group-wide on expiry, so a `make` gate's whole build tree dies with it.  `since`-narrowing and streaming progress are follow-on scope, the latter because the transport has no notification plumbing yet.
