...
It is scored on the repository's benchmark suite (`data/benchmarks/`): 43 obligations with committed gold solutions, 22 from the Agda standard library and 21 from agda-algebras, each tagged with a difficulty tier.  Every run writes its results in the same JSON Lines schema the project's other evaluators use, so a search result can sit in a table beside a language model's.
...
A fixture's `searchStatus` says how its search ended: `solved` (the final check passed), `exhausted` (every state was expanded, or the depth bound cut a live frontier, and the outcome says which), or `budget_exceeded` (the probe budget ran out with work remaining).  The distinction is the difference between "no term-mode proof exists within this depth" and "we stopped looking", and the measurements in the ADR lean on it.
