...
+  Launch the client from any shell, inside one of this flake's devShells included.  The launcher clears the `LD_LIBRARY_PATH` those shells export before it enters `nix develop`, because the profile `nix` otherwise loads the shell's older `libssl` and dies before any JSON-RPC (issue #153; the client showed it as `CONNECTION_CLOSED`).  Set `AGDA_MCP_BIN` to run a prebuilt server binary, for instance from a worktree that has not built its own; without it the launcher runs what `cabal list-bin exe:agda-mcp` names, and says how to build it when that binary does not exist yet.
