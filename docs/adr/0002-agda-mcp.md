...
**Where the contract is stated ([#191], PR [#193])**.  A client puts every tool's description and schema, and the server's `initialize` instructions, in its model's context on every turn, and Claude Code 2.1.282 cuts each description, and the instructions, at 2,048 characters.  Measured before [#191]: eleven of the fourteen descriptions ran to 3,193 to 7,648 characters, so 27,808 characters of contract never reached a model, `fill_hole`'s `status` rule, `check_file`'s hole listing, and the wrong-tree refusal on two tools among them; and about 37,500 of the 77,603 characters of `tools/list` repeated text stated elsewhere in the same list.  So the contract is now split by who shares it and stated once.

+  **The instructions** (1,982 characters) carry what every tool shares: the two lanes and each one's cost (the exit-code rule stated of `success` alone, since `fill_hole`'s status tolerates open holes, `check_project`'s output can turn its gate red, and `get_goal`'s fallback exit judges nothing), `reload`, the path refusal, the tree every answer names and the wrong-tree refusal with its registry limit, `verbose`, how failures arrive, `checkedFromSource`, and the hole model's coordinates; they name only the tools the server exposes.
+  **Each description** carries its own tool's contract and nothing else, every one under the cap (the longest, `check_project`'s, is 1,960 characters), and names another tool only when that tool is exposed (a subset hiding `check_file` gets `get_diagnostics`' whole contract); the suite asserts both, for every single-tool subset.
...
`tools/list` fell from 77,603 characters to 24,094 (plus 1,982 of instructions; the arms below ran the same `tools/list` and a 1,989-character version of the instructions, before review fixes scoped their exit-code rule to `success`), and the surface as delivered to Sonnet 5, the first-turn difference against a one-tool probe server, from 16,230 tokens a turn to 9,217, or 3,583 with the four verdict tools alone exposed.  On Sonnet 5 arms at the [#162] caps (runs `arm-surface-mcp-1` and `-2`, `arm-surface-both-1`, `arm-verdict-mcp-1` and `-2`, 2026-09-26, zero anomalies; [`reports/agent-bench/README.md`]) the trim cut a server arm's cached tokens a turn by about a third and its cost by 17 to 19 %, and left the tool mix and the restatements as they were.  With the four verdict tools alone the per-turn context fell below the shell arm's (about 9,600 cached tokens a turn against 10,060), yet the arm cost USD 3.78 and 3.84 against 2.88, because it wrote more and took 150 more turns, most of them reading the library by trial (111 and 97 failed reads).  That arm solved 52 and 53 and restated 1 and 2, against the full surface's 48 and 45 and 7 and 8, and the transcripts show the difference is the route and not better proving: with the search tools the subject found the library's lemma by name and cited it; without them it read the original's module and wrote the proof out, which the judge counts as solved, and on the 34 rows with no original every arm solves 31 to 34.
...
| 5 | For the per-file tools, `success` is a function of the exit code alone; every batch verdict carries `verdict`, `command`, `project` | Adopted ([#72], [#76], PR [#95]); in full on request since [#184] (PR [#190]) | Stand-in binary test; field report § 2 and § 6 |
...
| 10 | Project resolution stays local, per call, echoed; a wrong tree is a refusal before spawn | Adopted ([#76], PR [#95]); root and rootSource by default, the rest on request, since [#184] (PR [#190]) | No Agda query exists; the environment note's reproduction |
