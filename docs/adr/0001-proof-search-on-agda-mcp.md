...
| 17 | Ranking is measured offline against index ground truth (`target:`, `restates:`), by replaying the shared pool pipeline on recorded goals | Adopted ([#19]) | The instrument reproduces run `ctx-probe-1`'s pool counts and accepted order; `RetrievalRecallSpec` |
| 18 | `idf-unfold` is the deterministic scorer; the default stays `token-overlap` so published numbers reproduce | Adopted ([#19]) | 9/31 fair targets and 16/21 originals in the top eight against 1/31 and 5/21; every rule pinned on corpus rows in `RetrieveSpec` |
