...
The terms.  A *subject* is one fresh, non-interactive Claude Code session (`claude -p`) given a staged copy of one obligation, the thirteen `agda-mcp` tools with the row's library corpus loaded, and Read and Edit on that one file, under a turn cap, a cost cap, and a wall cap.  An *arm* is one model over the whole suite, one subject per obligation.  The *judge* is the gold verifier's own `agda` invocation, with `--safe` added because every committed gold passes under it, on the file the subject leaves behind, after three syntactic gates: *preservation* (the module line, every original import line, and the type signature byte-identical; added import lines allowed and logged), *escape* (no postulate, no `trustMe`, no pragma), and *holes* (none left).  A file that passes every gate but names the library's own lemma for the statement it was asked to prove is *restated*: its own column, never counted as solved, the agent-side twin of the loop's target exclusion (the original is the index row's `restates:` tag, PR [#152]'s qualified corpus name, where the row has one; on the standard-library rows, which carry none, it is the index's module and name, and the agda-algebras fixtures' primes agree with their tags on every row; the haystack tier has no original by construction).
...
+  **Same verifier, same suite, same shape**.  The judge's `agda` command is `GoldVerifier`'s, so an agent's solve and a loop's solve mean one thing; the report is the loop's (`perTier`, `perStratum`, one outcome per obligation) plus per-tool call counts, turns, wall, cost, and tokens; `results.jsonl` holds one `eval-proof-completion.v0` row per `fill_hole` the subject probed.
...
**Status**.  Adopted and measured ([#154]); the harness is `struxdriver.agentbench` (`make agent-bench`), its judge pinned by `JudgeSpec` on every committed obligation and gold, its transcript reader by `TranscriptSpec` on a captured stream, its Agda gate by `AgentBenchIntegrationSpec`; the transcripts, verdicts, and final files of every quoted run are under [`reports/agent-bench/README.md`].  The arms were judged before [#152] merged, with the original read off the fixture's prime; the gate now reads the `restates:` tag first and re-judges every archived row to the same verdict.
...
| 20 | The agent in the loop is a second instrument under the loop's verifier and report shape: one fresh session per obligation with the thirteen tools and one file, isolation verified per transcript, a restated column for a looked-up lemma, turns and tool calls as the comparable columns | Adopted ([#154]) | Sonnet 5 46/55 and Opus 5 54/55 against the loop's 14/55, 8 and 1 restated, zero anomalies; `JudgeSpec` over all 55 pairs, `TranscriptSpec` on a captured stream, `AgentBenchIntegrationSpec` (§ 9) |
