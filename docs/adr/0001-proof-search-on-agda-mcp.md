...
| 19 | `idf-unfold` is the deterministic scorer; the default stays `token-overlap` so published numbers reproduce | Adopted ([#19]) | 9/31 fair targets and 16/21 originals in the top eight against 1/31 and 5/21; every rule pinned on corpus rows in `RetrieveSpec` |
