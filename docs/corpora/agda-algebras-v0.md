...
+  **1,146 rows are shadowed under `prettyQname`**.  655 qualified names occur more than once (worst: `Classical.Structures.Ring.absurdlambda`, 28 times), because normalization drops anonymous module segments and distinct definitions collapse onto one name.  A consumer keyed by `prettyQname` — `agda-mcp` keeps the last occurrence — therefore indexes 10,520 of the 11,666 rows.  Every row is still in the file; use `qname` when identity matters.  Tracked by issue #144 (this card said #53 until the #131 review caught the mispointer; #53 is the unrelated Agda-Nix registration task).
