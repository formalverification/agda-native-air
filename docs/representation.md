...
+  **Legacy migration**.  Legacy ETL components should no longer
...
| `ports` | object | **[Planned v1.0]** Optional interface extraction (`inputs`/`outputs`), see §6. May be emitted by backend or derived by ETL. Tracked in [Issue #15](https://github.com/formalverification/agda-native-air/issues/15). |
...
**Tracking**.  Implementation tracked in [Issue #15](https://github.com/formalverification/agda-native-air/issues/15).
