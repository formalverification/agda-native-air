...
> This skill covers the M1-5 baseline benchmark introduced by Issue #13 (PR #50).  The `data/benchmarks/` tree, `docs/benchmarks/taxonomy.md`, and the example fixtures named below are created by that work; on a branch without it, add them as you extend the suite.

## 1. Choose and classify

+  Pick an obligation with a stable module path, a clear hole identifier, and a known gold solution.  Classify it into a tier using the taxonomy (`docs/benchmarks/taxonomy.md`, added by Issue #13): `routine` (Tier 1), `compositional` (Tier 2), or `non-obvious` (Tier 3).
