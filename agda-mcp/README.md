...
| `modulesChecked` | How many distinct modules `agda` re-typechecked from source, so how much of the project was actually rebuilt: 0 on a fully warm gate, the whole import graph on a cold one, and on a timeout, how far it got.  Absent when the gate's own command line carries `--trace-imports=0` — the flag silences the very lines this counts, so what could still be counted is a floor rather than a total (issue #114).  A `make` gate that hides the flag inside a recipe is beyond the server's view, and there the count stays a best-effort read. |
