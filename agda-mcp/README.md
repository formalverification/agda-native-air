...
`problem` is one of four: `missing`; `notAFile` (you passed a directory); `notRegularFile` (you passed a FIFO, socket, or device, named under `detail` — reading one blocks or never ends, and it would do so *before* `--timeout` has anything to bound, so the server refuses to open it at all); or `unreadable` (a regular file whose read was refused, with the syscall's own words under `detail`).  All four arrive as `isError` tool results — content your client hands its model — never as a JSON-RPC error.
