...
#   must exist: `cabal list-bin` builds nothing, so a fresh worktree needs
#   `cabal build exe:agda-mcp` first, and the message says so instead of the
#   bare "No such file" the client would otherwise see.
#
# Usage (from anywhere):
...
exec nix develop "${REPO_ROOT}#backend" --command \
  bash -c '
    ...
    [ -x "$BIN" ] || {
      echo "agda-mcp: server binary not found or not executable: $BIN" >&2
      echo "agda-mcp: build it with:  cd '"${REPO_ROOT}"'/agda-mcp && cabal build exe:agda-mcp" >&2
